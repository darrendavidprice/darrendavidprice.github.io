...
- **Canonical build** lives at `/home/claude/build/` in the project
  ...
  restore from the latest `darrendavidprice-site-full_vX.Y.Z.zip` (see §7 and
  §9) — the zip is fully up to date and requires no post-extraction fixes.

- **Current internal version: v3.4.2.** The README header carries the
  authoritative version log; bump it on each shipped batch.

- **v3.4.2 deltas (this batch).** `talks.yml` +42 hand-curated 2023–2026 entries
  (440 → **482**; header year range now 2004–2026). New Scientist tours unified to
  `public`; two Feb-2024 DRD2 talks set `internal`; four near-duplicate April-2024
  SOLAIRE "vision…" entries retitled "SOLAIRE working meeting updates". 14/42 carry
  agenda links. `about/index.html` six inline external links (ATLAS, CERN, DarkSide
  split → LNGS + internal DM link, Gran Sasso → LNGS, St. Joseph's school, Port
  Talbot). Example Writing post given `published: false` (empty-states cover home/
  footer/`/writing/`). TODO talk-curation item rewritten to cover tags/links/dates
  of the pre-2024 parsed entries.

- **v3.4.1 deltas (earlier batch).** CV `roles.yml` edits (IPPOG end-dated; ODROE
  co-lead removed; B-Physics group reworded; Editorial Board → 2010– "member and
  chair, ATLAS and DØ"; BSF-2027 chair added). `team.yml` 13 research-focus
  rewrites + Dark-matter tag on Pickles. `team/index.html` stray-"Now" guard now
  `{% if a.now.label != blank %}` (empty string is truthy in Liquid). **Teaching
  page "MPhys projects" count is now computed live** from `team.yml` MPhys roles
  (one project/assessment per student, `split: " & "`), not a hardcoded number;
  `supervision.pdra` gained Ash Ritchie-Yates → 8 postdocs (matches the team
  page). **`engagement/index.html` rebuilt** (dated leadership roles-list, broadened
  activities, Media section, explorers under their own "Interactive tools"
  heading). New `assets/images/engagement/` (2025 Rob Watson hero + 3-photo grid;
  3 small event thumbnails). New CSS: `.photo-grid`, `.event-fig`, `.event-block`.

- **All major systems are shipped and stable.** Core site (11-item nav, 8
  research themes, 4-section publications, 482-talk record, news timeline,
  team page, /join, /cv, /about, writing infra), both dark-matter explorers,
  and the SEO/sitemap/404 infrastructure are all in place. Per-area detail:
  - **Explorers** — full architecture in §11 (Sensitivity) and §12 (Recoil).
    These are the two pieces most likely to need careful re-reading before
    edits; treat §11/§12 as the source of truth, not this summary.
  - **Publications** — 4 CV-style sections; explainers on PRIMARY only;
    optional `explainer_image` + `explainer_caption` figure support (worked
    examples on `p-ds50-lowmass-wimp-2023` and `p-ssww-majorana`).
  - **Shipped-batch history** lives in TODO's "Done" section (by version),
    not here — check there for what landed when.

- **Open work** is tracked in TODO.md and the §8 rough-edges list. Pick up
  from there.
...
### `talks.yml` — flat list of 482 entries
...
| **Presentations** | 482 entries. Title -> agenda link; PDF icon -> slides. Internal talks hidden by default. |
...
| **Structured data (JSON-LD)** | Hand-rolled `_includes/structured-data.html` in `<head>` of default.html (consistent with the no-`jekyll-seo-tag` decision). Single `@graph`, shared `@id` anchors (`#website`, `#person`). `WebSite` (full on home, slim elsewhere) + `Person` on every page; `ProfilePage` via `jsonld_type: ProfilePage` front matter (on /about/); `SoftwareApplication` via a `jsonld_software:` front-matter map (on both explorers); generic `WebPage` on other non-home pages; `BlogPosting` on posts gated behind `site.enable_blog_jsonld` (OFF until real Writing posts). `Person.sameAs` is the curated `site.author.same_as` list in `_config.yml` — the disambiguation payload; keep it complete. `jobTitle` = "Professor of Particle Physics and Associate Dean". `BreadcrumbList` intentionally omitted — paths are short (per the hawksley.dev guide, a minor gain). v3.4.0. |
...
| `/mnt/user-data/uploads/darrendavidprice-site-full_vX.Y.Z.zip` | **Current full snapshot — the restore source.** Versioned per batch; use the latest. Fully up to date; no post-extraction fixes needed. |
| CV — `DarrenPrice_CV_and_MelbourneCoverStatement_June2026.docx` | Master content source for team/grants/roles/publications. Lives in **project knowledge**, not uploads. |
| DS-20k TDR (`DARKSIDE-CSN2-TDR-2112`, Fig 2) | Source for `REF_DS20K` data in the sensitivity explorer. **Not currently in uploads** — re-upload if the explorer reference curve needs reworking. |
| `/mnt/user-data/outputs/*.zip` | Previous deliverables. |

> **Note:** `/mnt/user-data/uploads/` does not persist across chats and is
> typically empty at the start of a new session except for whatever the user
> re-uploads. Don't assume the CV PDF or TDR PDF are present.
...
3. **Restore the canonical build if missing:**
   ...
   unzip /mnt/user-data/uploads/darrendavidprice-site-full_vX.Y.Z.zip
   ```
   (Use the actual latest versioned filename the user uploaded.)
...
Upload the latest `darrendavidprice-site-full_vX.Y.Z.zip` at the start of the
new chat. The CV docx lives in project knowledge; the DS-20k TDR PDF does not
persist and only needs re-uploading if the explorer reference curve is being
reworked.
...
**Limit-recovery validation** (harness `/tmp/validate.js`). Limit
...
- **v2 enhancements** still in TODO.md: interaction-type selector,
  ...
  comparison. The efficiency turn-on is now DONE.
